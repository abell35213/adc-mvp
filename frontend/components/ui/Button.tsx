...
export function Button({variant="primary",size="md",iconBefore,iconAfter,loading=false,loadingLabel="Loading",fullWidth=false,disabled: isDisabled,children,className,type="button",...props}:ButtonProps){
 return <button type={type} disabled={isDisabled||loading} aria-busy={loading||undefined} className={cn("inline-flex cursor-pointer items-center justify-center gap-2 rounded-md border font-medium",focusRing,transition,disabled,sizes[size],variants[variant],fullWidth&&"w-full",className)} {...props}>{loading?<span aria-hidden="true" className="size-4 animate-spin rounded-full border-2 border-current border-r-transparent motion-reduce:animate-none"/>:iconBefore}<span>{loading?loadingLabel:children}</span>{!loading&&iconAfter}</button>;
}
