  ...
  return (
    <Card variant="subtle">
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-sm font-semibold text-text-primary">Filters</h2><p className="text-xs text-text-secondary">Search available case, vehicle, driver, owner, and status fields returned by the queue API.</p></div><Button variant="secondary" size="sm" onClick={onReset} disabled={loading || activeCount === 0}>Clear filters{activeCount > 0 ? ` (${activeCount})` : ""}</Button></div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          <FormField id="dashboard-search" label="Search"><Input id="dashboard-search" value={filters.search} onChange={(e) => onChange({ ...filters, search: e.target.value })} placeholder="Case, vehicle, driver, owner" disabled={loading}/></FormField>
