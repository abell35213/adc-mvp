...
function toWeatherMetrics(weather: unknown): Array<{ key: string; value: string }> {
  ...
  return Object.entries(weather)
    ...
    .map(([key, value]) => ({ key: key.replaceAll("_", " "), value: formatWeatherValue(value) }));
}
...
export default function IncidentDetailClient() {
  ...
  const nextActionTab: WorkspaceTab =
    view?.nextAction.kind === "missing_evidence"
      ? "evidence"
      : view?.nextAction.kind === "download" || view?.nextAction.kind === "generate"
        ? "documents"
        : "overview";
      ...
      <div className="space-y-6">
        <header className="rounded-xl border border-border-default bg-surface p-6 shadow-bordered">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2 lg:justify-end">
              <Button onClick={() => changeTab(nextActionTab)}>{view.nextAction.label}</Button>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0 space-y-5">
            ...
            {selectedTab === "overview" ? <OverviewTab view={view} incident={incident} waitingOnDriver={waitingOnDriver} protocol={driverProtocolSettings?.instruction_source} onChangeTab={changeTab} nextActionTab={nextActionTab} /> : null}
          ...
          <aside className="space-y-4 xl:sticky xl:top-6 xl:self-start" aria-label="Case action rail">
            <Card><CardHeader title="Next best action" description={view.nextAction.reason}/><CardContent><Button fullWidth onClick={() => changeTab(nextActionTab)}>{view.nextAction.label}</Button></CardContent></Card>
...
function OverviewTab({ view, incident, waitingOnDriver, protocol, onChangeTab, nextActionTab }: { view: NonNullable<ReturnType<typeof buildIncidentWorkspaceViewModel>>; incident: IncidentDetail; waitingOnDriver: boolean; protocol?: string | null; onChangeTab: (tab: string) => void; nextActionTab: WorkspaceTab }) {
