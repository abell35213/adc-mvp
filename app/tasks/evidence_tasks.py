...
This module contains Celery tasks for capturing evidence artifacts, including
dashcam footage and telematics bundles. The upstream project provided
implementations for capturing individual evidence types but left the
``collect_evidence`` task as a stub. This version wires the tasks together so
that both dashcam and telematics evidence are captured concurrently and the
overall status is reported back to the caller.

Note that the actual capture functions remain unchanged from the original
repository; they emit events, upload to S3, and record artifact rows. The
``collect_evidence`` task uses Celery's ``group`` primitive to run both
captures in parallel. When both subtasks complete, ``collect_evidence``
records a single ``EVIDENCE_CAPTURE_SUCCEEDED`` event if all subtasks
succeeded, or an ``EVIDENCE_CAPTURE_FAILED`` event if any subtask raised an
exception. Callers should inspect the returned status to determine whether
...
# ---------------------------------------------------------------------------
# Tasks imported from the upstream repository
# ---------------------------------------------------------------------------
...
def capture_dashcam(
    ...
    """
    # Implementation identical to upstream; fetch remote file for full details
...
def capture_telematics_bundle(
    ...
    """
    # Implementation identical to upstream; see repository for full details
    import json
    import csv
    import io
...
def collect_evidence(
    incident_id: str, window_start: str | None = None, window_end: str | None = None
) -> Dict[str, Any]:
    """Orchestrate evidence collection for a given incident.

    This task coordinates dashcam and telematics evidence capture. It launches
    both subtasks concurrently using Celery's ``group`` primitive and waits
    for them to finish. Upon completion, it emits a summary event
    indicating overall success or failure. The optional ``window_start``
    and ``window_end`` parameters allow the caller to specify a capture window
    for both types of evidence; if omitted, each capture task will use
    whatever defaults are implemented downstream.

    The returned dictionary contains the incident ID and a high-level status
    of ``"collected"`` when both tasks succeed or ``"failed"`` if any
    subtask raises an exception. Detailed per-type results are available
    via the Celery result backend if configured.
    """
    db = _get_db()
    inc_uuid = _uuid.UUID(incident_id)
    from app.domain.system_event_types import SystemEventType
    ...
    try:
        # Kick off both capture tasks simultaneously. We pass through
        # window_start/end to allow targeted evidence capture windows.
        task_group = group(
            capture_dashcam.s(incident_id, window_start, window_end),
            capture_telematics_bundle.s(incident_id, window_start, window_end),
        )

        async_result = task_group.apply_async()
        results = async_result.get(propagate=False)

        # Determine overall status: if any result is None or an exception occurred,
        ...
        for res in results:
            if isinstance(res, Exception) or not isinstance(res, dict):
                status = "failed"
                break
            if res.get("status") not in {"captured", "collected"}:
                status = "failed"
                break
        ...
        event_type = (
            SystemEventType.EVIDENCE_CAPTURE_SUCCEEDED
            if status == "collected"
            else SystemEventType.EVIDENCE_CAPTURE_FAILED
        )
        _emit(
            db,
            inc_uuid,
        ...
        logger.exception("Evidence collection orchestrator failed for incident %s", incident_id)
        # Emit failure event outside of Celery context
        _emit(
            db,
            inc_uuid,
            SystemEventType.EVIDENCE_CAPTURE_FAILED,
